...
if ( ! class_exists( 'Tax_Meta_Class' ) ) :
     ...
     */
        ...
        <img src="';
					...
					echo '" alt="' . __( 'Remove', 'coaching' ) . '" title="' . __( 'Remove', 'coaching' ) . '" id="remove-' . $field['id'] . '"></div>';
					$c = $c + 1;
			...
			echo '" alt="' . __( 'Add', 'coaching' ) . '" title="' . __( 'Add', 'coaching' ) . '" id="add-' . $field['id'] . '"><br/></div>';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
		...
		public function show_field_image( $field, $meta ) {
			...
			wp_enqueue_script( 'jquery-ui-sortable' );
			wp_enqueue_media();
			$std   = ( ! empty( $field['std'] ) ) ? $field['std'] : array( 'id' => '', 'url' => '' );
			$name  = esc_attr( $field['id'] );
			$value = isset( $meta['id'] ) ? $meta : $std;
