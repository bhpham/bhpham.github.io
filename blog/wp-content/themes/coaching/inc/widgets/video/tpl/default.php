		...
		<div class="video-container content-asset">
			...
			if ( $src_poster ) { ?>
				<div class="btns">
					...
					<p class="watch"><?php echo esc_html__( 'Watch the video', 'coaching' ) ?></p>
				</div>
				<?php
...
<?php } else {
	...
	<div class="flexslider slide-video <?php echo esc_attr( $layout ) ?>">
		<ul class="slides">
			...
			$j < $i;
			$j ++ ) { ?>
			<li data-thumb="<?php echo esc_attr( $arr_img[$j] ); ?>">
				<div class="content">
					<div class="play-button"><i class="fa fa-play"></i></div>
					<iframe id="video-gallery" height="500" src="<?php echo esc_url( $videos[$j] ); ?>" allowfullscreen="" style="border: 0px;"></iframe>
					<img class="bg-gallery-video" src="<?php echo esc_url( $arr_img[$j] ); ?>" />
				</div>
			</li>	
				<?php } ?>
		</ul>
