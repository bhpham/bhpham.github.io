...
if ( ! class_exists( 'Thim_Core_Installer' ) ) {
	...
	class Thim_Core_Installer {
		...
		public function notice_install() {
            ...
            <div class="notice notice-success">
                <h3><?php printf( __( '%s Theme notice!', 'coaching' ), $theme['name'] ) ?></h3>
                <p>
					<?php printf( __( 'Installed the theme successfully. <a href="%s">Enable Thim Core to start now!</a>', 'coaching' ), $link ) ?>
                </p>
            </div>
			<?php
		}
		...
		public function add_menu_installer() {
			...
			add_theme_page(
				__( 'Thim Core Installer', 'coaching' ),
				'Thim Core Installer',
				'edit_theme_options',
				$this->slug,
		...
		private function setup_content() {
            ...
            <div class="thim-setup-content">
                ...
                <div class="header text-center">
                    <h1 class="title"><?php esc_html_e( 'Thim Core Installer', 'coaching' ) ?></h1>
                    <div class="sub-title">
						<?php printf( __( 'Thank you for choosing %1$s Theme!', 'coaching' ), $theme['name'] ); ?>
                    </div>
                </div>
				...
				<?php else: ?>
                    ...
                    <div class="thim-core-active">
                        <h2><?php esc_html_e( 'Your site have already installed Thim Core!', 'coaching' ); ?></h2>

                        <div><?php printf( __( '<button data-href="%s" class="thim-button-link button button-primary">Return to Dashboard</button>', 'coaching' ), admin_url( 'admin.php?page=thim-dashboard' ) ) ?></div>
                    </div>
		...
		public function prepare_installation() {
            ...
            <div class="install-container text-center">
                <button class="thim-button-link button button-primary" data-href="<?php echo esc_url( $link ) ?>"><?php esc_html_e( 'Install and activate', 'coaching' ) ?></button>
            </div>
			<?php
		}
		...
		public function step_install() {
			...
			?>
            <h3><?php esc_html_e( 'Installing Thim Core', 'coaching' ) ?></h3>
            ...
            <h3><?php esc_html_e( 'Activating Thim Core', 'coaching' ) ?></h3>
            <div class="updating-message notice notice-success">
                <p><?php esc_html_e( 'Activating...', 'coaching' ) ?></p>
            </div>
			<?php
		...
		public function step_activate() {
			...
			if ( $plugin->activate( true ) || $plugin->is_active() ) {
				?>
                <h3><?php esc_html_e( 'Activating Thim Core successfully!', 'coaching' ) ?></h3>
                <div class="updating-message notice notice-success">
                    <p><?php printf( __( 'You are redirecting to %s theme dashboard...', 'coaching' ), $theme['name'] ) ?></p>
                </div>
				<?php
			...
			?>
            <h3><?php esc_html_e( 'Activating Thim Core failed!', 'coaching' ) ?></h3>
            <div class="notice notice-error">
                <p><?php esc_html_e( 'Something went wrong!', 'coaching' ) ?></p>
                <p>
                    <button data-href="<?php echo esc_url( $link ) ?>" class="thim-button-link button button-primary"><?php esc_html_e( 'Activate again', 'coaching' ) ?></button>
                </p>
            </div>
		...
		private function setup_environments() {
            ...
            <table class="widefat striped" cellspacing="0">
                <thead>
                <tr>
                    <th colspan="2"><?php esc_html_e( 'Configuration Check', 'coaching' ) ?></th>
                </tr>
                </thead>

                <tbody>
                <tr>
                    <td><?php esc_html_e( 'Server Info', 'coaching' ); ?></td>
                    <td><?php echo esc_html( $args['server_info'] ); ?></td>
                </tr>

                <tr>
                    <td><?php esc_html_e( 'PHP Version', 'coaching' ); ?></td>
                    <td>
						<?php
						if ( version_compare( $args['php_version'], '5.6', '<' ) ) {
							echo '<mark class="error"><span class="dashicons dashicons-warning"></span> ' . sprintf( __( '%1$s - We recommend a minimum PHP version of 5.6. See: %2$s', 'coaching' ), esc_html( $args['php_version'] ), '<a href="https://goo.gl/WRBYv3" target="_blank">' . __( 'How to update your PHP version', 'coaching' ) . '</a>' ) . '</mark>';
						} else {
                ...
                <tr>
                    <td><?php _e( 'cURL version', 'coaching' ); ?>:</td>
                    <td><?php echo esc_html( $args['curl_version'] ) ?></td>
                </tr>

                <tr>
                    <td><?php _e( 'Remote GET', 'coaching' ); ?></td>
                    <td><?php
						if ( $args['remote_get_successful'] ) {
							echo '<mark class="yes"><span class="dashicons dashicons-yes"></span></mark>';
						} else {
							echo '<mark class="error"><span class="dashicons dashicons-warning"></span> ' . __( 'wp_remote_get() failed. Please contact your hosting provider.', 'coaching' ) . ' ' . esc_html( $args['remote_get_response'] ) . '</mark>';
						} ?>
                    </td>
                </tr>

                <tr>
                    <td><?php _e( 'Plugins directory', 'coaching' ); ?></td>
                    <td>
                ...
                <tr>
                    <td><?php _e( 'Plugins directory writable', 'coaching' ); ?></td>
            ...
            <div class="notice notice-error">
                <p><?php printf( __( 'Please follow those instructions above to make sure your server is ready to use %s theme. If you need assistance, please get our support <a href="%s" target="_blank">here</a>.', 'coaching' ), $theme['name'], $theme['support'] ) ?></p>
            </div>
			<?php
		}
	...
	private function setup_header() {
        ...
        <head>
            ...
            <title><?php printf( __( 'Thim Core Installer &lsaquo; %s', 'coaching' ), $theme['name'] ) ?></title>
        ...
        <div class="footer text-center">
            <div class="brand">
				<?php esc_html_e( '&copy; ThimPress 2017. All rights reserved. Powered by WordPress.', 'coaching' ) ?>
            </div>
        </div>
        </body>
		<?php
